/*
二叉搜索树
    针对动态查找组织的数据结构，方便插入和删除
    非空左子树所有结点的值都小于其根结点
    非空右子树所有结点的值都大于其根结点

平衡二叉树(AVL树)
    搜索树结点不同的插入次序，将导致不同的深度和平均查找长度
    平衡因子：定义为左右两个子树的高度差，左 - 右
    平衡二叉树：任一结点平衡因子小于等于 1 的非空二叉搜索树
    给定结点数为 n 的平衡二叉树的最大高度为 log2(n)

平衡二叉树的调整
    RR旋转：麻烦结点在被破坏结点右子树的右子树上
    LL旋转：麻烦节点在被破坏结点左子树的左子树上，调整的时候考虑最下方的被破坏结点
    LR旋转：基本同上
    RL旋转：基本同上
*/
...
BinTree Find(ElementType X, BinTree BST)
{
	...
	else if(X > BST->Data)  // 比根结点大，去右子树查找
		return Find(X,BST->Right);
	else if(X == BST->Data) // 找到了
		return BST;
}
...
BinTree Delete(ElementType X, BinTree BST)
{
	...
	else if(X > BST->Data) // x 比当前结点值大，在右子树继续查找删除
